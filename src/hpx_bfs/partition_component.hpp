      ...
      public hpx::components::managed_component_base<partition>
    {
      ...
      public:
        ...
        void convert_graph_to_csr(const std::uint64_t nedges_in,
            ...
            csr_graph * const g){
          const std::size_t edge_buffer_size = (1<<27) / (2* sizeof(std::int64_t)) / size;  //128MB
          std::size_t edge_buffer = malloc(2* edge_buffer_size);
...
#define TEST_VISITED(v) ((visited[VERTEX_LOCAL((v)) / ulong_bits] & (1UL<<(VERTEX_LOCAL((V)) % ulong_bits))) != 0)
          ...
          std::size_t *outgoing_counts = (std::size_t *)malloc(size * sizeof(std::size_t));
